...
def validate_contact_no(user,contact_no):
    
    #user will be 'None' when creating user and will be an instance of 'User' while upadting user
    if(isinstance(user,User)):
        ...
        if(value_count>0):
            ...
            #excluding current instance's contact_no for uniqueness(see limitation of updating nested serializers)
...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    #request should contain "bio":null if user is not interested in adding 'bio'
    bio=serializers.CharField(max_length=100,allow_null=True)

    #django will 'REJECT' if provided in request
    is_auctioneer=serializers.BooleanField(read_only=True)
...
#username is read only because we are using username to search the user,so option of updating username should not be there
    ...
    def update(self,instance,validated_data):
        ...
        #to make sure it is not created:))
        instance.save(force_update=True)
